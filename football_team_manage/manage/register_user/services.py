...
def get_search(page, search):
    user = User.query.join(Roles).filter(Roles.name == 'register user', User.user_name.contains(search)).order_by(-User.id) \
        .paginate(page=page, per_page=3, error_out=True)
    if check_header():
        list = {}
        for item in user.items:
...
def get(id):
    ...
    user = User.query.join(Roles).filter(User.id == id, Roles.name == 'register user').first_or_404()
    if user:
...
def add():
    try:
        ...
        if validator != True:
            ...
            phone = data['phone']
            status = True
            role = Roles.query.filter_by(name='register user').first_or_404()
